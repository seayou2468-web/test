...
# I have a duplicate autoFetchAndMountDDIWithCompletion
implementation_start = content.find('@implementation DeviceConnectionManager')
parts = content[implementation_start:].split('@end')

# The implementation block should only have one of each method.
# Since my previous `cat` showed them back to back:
double_method = r'''- (void)autoFetchAndMountDDIWithCompletion:(void (^)(NSError *error))completion {
...
content = content.replace(double_method + "\n" + double_method, double_method)
