...
typedef struct MCInstallerClientHandle MCInstallerClientHandle;

typedef struct ManagedConfigurationClientHandle ManagedConfigurationClientHandle;
...
/**
 * Creates a new ManagedConfiguration client from an existing stream
 *
 * # Safety
 * `stream` and `client` must be valid non-null pointers.
 */
struct IdeviceFfiError *managed_configuration_new(struct ReadWriteOpaque *stream,
                                                  struct ManagedConfigurationClientHandle **client);

/**
 * Retrieves all configuration profiles from the device
 *
 * # Safety
 * `client` and `out_plist` must be valid non-null pointers.
 */
struct IdeviceFfiError *managed_configuration_get_profile_list(struct ManagedConfigurationClientHandle *client,
                                                               plist_t *out_plist);
 ...
 *
 * # Safety
 * `client` and `profile_data` must be valid non-null pointers.
 */
struct IdeviceFfiError *managed_configuration_install_profile(struct ManagedConfigurationClientHandle *client,
                                                              const uint8_t *profile_data,
                                                              size_t profile_len);

/**
 * Removes a configuration profile from the device
 *
 * # Safety
 * `client` and `identifier` must be valid non-null pointers.
 */
struct IdeviceFfiError *managed_configuration_remove_profile(struct ManagedConfigurationClientHandle *client,
                                                             const char *identifier);

/**
 * Frees a ManagedConfiguration client handle
 *
 * # Safety
 * `handle` must be a valid pointer.
 */
void managed_configuration_client_free(struct ManagedConfigurationClientHandle *handle);
...
/**
 * Automatically creates and connects to MCInstaller, returning a client handle
 *
 * # Safety
 * `provider` and `client` must be valid non-null pointers.
 */
struct IdeviceFfiError *mcinstaller_connect(struct IdeviceProviderHandle *provider,
                                            struct MCInstallerClientHandle **client);
 ...
 * `client` and `out_plist` must be valid non-null pointers.
 */
struct IdeviceFfiError *mcinstaller_get_profile_list(struct MCInstallerClientHandle *client,
                                                     plist_t *out_plist);
 ...
 * `client` and `profile_data` must be valid non-null pointers.
 */
struct IdeviceFfiError *mcinstaller_install_profile(struct MCInstallerClientHandle *client,
                                                    const uint8_t *profile_data,
                                                    size_t profile_len);
 ...
 * `client` and `identifier` must be valid non-null pointers.
 */
struct IdeviceFfiError *mcinstaller_remove_profile(struct MCInstallerClientHandle *client,
                                                   const char *identifier);

/**
 * Frees an MCInstaller client handle
 *
 * # Safety
 * `handle` must be a valid pointer.
 */
void mcinstaller_client_free(struct MCInstallerClientHandle *handle);
