...
@interface DeviceConnectionManager () {
    ...
    BOOL _syslogRunning;
    BOOL _heartbeatConnecting;
    uint16_t _baseRsdPort;
    uint16_t _appServicePort;
    BOOL _modernInfrastructureFailed;
    BOOL _discoveryDone;
}
...
- (instancetype)initWithDelegate:(id<DeviceConnectionManagerDelegate>)delegate {
    ...
    if (self) {
        ...
        _heartbeatQueue = dispatch_queue_create("com.test.heartbeatQueue", DISPATCH_QUEUE_SERIAL);
    }
    return self;
}
...
- (void)performConnectWithData:(NSData *)data withToken:(NSInteger)token {
    ...
    if (!check()) return;
    err = idevice_tcp_provider_new((const struct sockaddr *)&addr, _pairingFile, "mdk-dashboard", &_provider);
    if (err || !_provider) {
    ...
    if (!err && val) {
        char *v = NULL;
        plist_get_string_val(val, &v);
        if (v) { [self log:[NSString stringWithFormat:@"[CONN] Device Version: %s", v]]; plist_mem_free(v); }
        plist_free(val);
    } else if (err) { [self log:[NSString stringWithFormat:@"[WARN] GetVersion: %s", err->message]]; idevice_error_free(err); }
}
...
- (void)ensureServiceConnected:(NSString *)name {
    ...
    if ([name isEqualToString:@"SB"] && !_springboard) {
    ...
    } else if ([name isEqualToString:@"ModernInfrastructure"]) {
        if (_adapter || _modernInfrastructureFailed) return;
        ...
        if (err) { _modernInfrastructureFailed = YES; }
    } else if ([name isEqualToString:@"ModernDiscovery"]) {
        [self ensureServiceConnected:@"ModernInfrastructure"];
        if (!_adapter || _discoveryDone || _modernInfrastructureFailed) return;
        struct ReadWriteOpaque *socket = NULL;
        err = adapter_connect(_adapter, _baseRsdPort, &socket);
        if (!err && socket) {
            struct RsdHandshakeHandle *handshake = NULL;
            err = rsd_handshake_new(socket, &handshake);
        ...
        [self ensureServiceConnected:@"ModernDiscovery"];
        uint16_t port = (_appServicePort > 0) ? _appServicePort : _baseRsdPort;
        if (_adapter && port > 0) {
            struct ReadWriteOpaque *socketAS = NULL;
            err = adapter_connect(_adapter, port, &socketAS);
            if (!err && socketAS) {
...
- (void)cleanupInternal {
    ...
    _baseRsdPort = 0;
    _appServicePort = 0;
    _heartbeatConnecting = NO;
    _modernInfrastructureFailed = NO;
    _discoveryDone = NO;
...
- (void)heartbeatTick {
    dispatch_async(_heartbeatQueue, ^{
        ...
        if (!self->_heartbeat && !self->_heartbeatConnecting) {
             self->_heartbeatConnecting = YES;
             struct IdeviceFfiError *connErr = heartbeat_connect(self->_provider, &self->_heartbeat);
             self->_heartbeatConnecting = NO;
             if (connErr) { idevice_error_free(connErr); self->_heartbeat = NULL; return; }
        }
        if (self->_heartbeat) {
...
- (void)fetchHomeScreenWallpaperWithCompletion:(void (^)(UIImage *image, NSError *error))completion {
    dispatch_async(_connectionQueue, ^{
        ...
        void *buf = NULL; size_t len = 0;
        struct IdeviceFfiError *err = springboard_services_get_home_screen_wallpaper_preview(self->_springboard, (void **)&buf, &len);
        if (!err && buf) {
...
- (void)fetchLockScreenWallpaperWithCompletion:(void (^)(UIImage *image, NSError *error))completion {
    dispatch_async(_connectionQueue, ^{
        ...
        if (!self->_springboard) { dispatch_async(dispatch_get_main_queue(), ^{ completion(nil, [NSError errorWithDomain:@"SB" code:-1 userInfo:@{NSLocalizedDescriptionKey: @"Not connected"}]); }); return; }
        void *buf = NULL; size_t len = 0;
        struct IdeviceFfiError *err = springboard_services_get_lock_screen_wallpaper_preview(self->_springboard, (void **)&buf, &len);
        if (!err && buf) {
            UIImage *img = [UIImage imageWithData:[NSData dataWithBytes:buf length:len]];
            idevice_data_free((uint8_t *)buf, (uintptr_t)len);
            dispatch_async(dispatch_get_main_queue(), ^{ completion(img, nil); });
...
- (void)autoFetchAndMountDDIWithCompletion:(void (^)(NSError *error))completion {
    dispatch_async(_connectionQueue, ^{
        ...
        if (!ecidErr && chipIdPlist) {
             plist_type t = plist_get_node_type(chipIdPlist);
             if (t == PLIST_INT) {
        ...
        if (versionPlist) {
            char *v = NULL; plist_get_string_val(versionPlist, &v);
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{ completion(nil); });
    });
}
